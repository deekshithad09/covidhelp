...
<body>
	...
	<script>	
		var websocket=new WebSocket("ws://localhost:8080/CovidHelp/VitalCheckEndPoint");
			...
			if(jsonData.message!=null)
				{
					...
					row.innerHTML="<td>"+details[0]+"</td><td>"+details[1]+"</td><td>"+details[2]+"</td><td><button class=\"btn btn-danger btn-sm\" onclick=\"sendInstructions('"+details[0]+"','"+details[1]+"','ambulance')\">Summon Ambulance</button><button type=\"button\" class=\"btn btn-primary btn-sm\" onclick=\"sendInstructions('"+details[0]+"','"+details[1]+"','medication')\">Suggest Medication</button></td>";
				}
		}
		function sendInstructions(username,phonenumber,message)
		{
			if(message=='medication')
			{
				...
				document.getElementById("submit_btn").addEventListener("click",function(){
					...
					websocket.send(username+','+phonenumber+','+message+','+medicine+','+description);
					medicine_name.value="";
			...
			{
				websocket.send(username+','+phonenumber+','+message);	
			}
		}
	</script>
</body>
